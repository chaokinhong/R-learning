...
blue_spheres <- 30
green_spheres <- 20
sample_without_replacement <- data.frame(Trial=1:num_of_trials) %>% mutate(Sample_sphere = map(.x=Trial,~sample(100,10,replace = FALSE)))

sample_without_replacement <- sample_without_replacement %>% mutate(nums_red=map_dbl(.x=Sample_sphere,~sum(.x<31)),num_blue=map_dbl(.x=Sample_sphere,~sum(.x>31 & .x<81)),num_green=map_dbl(.x=Sample_sphere,~sum(.x > 81 & .x<101)))

red <- sample_without_replacement %>% pull(nums_red)
blue <- sample_without_replacement %>% pull(num_blue)
green <- sample_without_replacement %>% pull(num_green)
min_x <- pmin(red,blue,green)
counts <- map_dbl(.x=min_x,~sum(.x==0))
count <- 0
for(i in counts){
  if(i==0){
    count<-count+1
